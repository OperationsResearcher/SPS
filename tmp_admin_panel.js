...
async function confirmDelete(title, text) {
    // SweetAlert varsa kullan, yoksa tarayıcı confirm'ünü kullan
...
function createNewOrganization() {
    ...
    .finally(() => {
        ...
        submitBtn.innerHTML = '<i class="fas fa-save me-2"></i>Kurum Oluþtur';
    });
}
...
function renderUserPagination() {
    ...
    if (!paginationContainer) {
        // Pagination container yoksa oluþtur
        const tableContainer = document.querySelector('.table-responsive');
        if (tableContainer && tableContainer.parentElement) {
            const paginationDiv = document.createElement('div');
            paginationDiv.id = 'userPagination';
            paginationDiv.classAd = 'd-flex justify-content-between align-items-center mt-3';
    ...
    if (totalPages <= 1) {
        document.getElementById('userPagination').innerHTML = `
            <div class="text-muted small">Toplam ${adminUsersCache.length} kullanıcı</div>
        `;
        return;
    }
    let paginationHTML = `
        <div class="text-muted small">
            Toplam ${adminUsersCache.length} kullanıcı - Sayfa ${currentUserPage} / ${totalPages}
        </div>
        <nav>
    ...
    paginationHTML += `
        <li class="page-item ${currentUserPage === 1 ? 'disabled' : ''}">
            <a class="page-link" href="#" onclick="changeUserPage(${currentUserPage - 1}); return false;">Çnceki</a>
        </li>
    `;
    // Sayfa numaraları
...
function updateUser() {
    if (!currentDüzenleUserId) {
        showToast('Güncellenecek kullanıcı bulunamadı.', 'error');
        return;
    }
    const csrf = document.querySelector('meta[name="csrf-token"]')?.content || '';
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (response.ok) {
          showToast(`Kullanıcı başarıyla silindi: ${data.message}`, 'success');
...
function createNewUser() {
    ...
    if (formData.password.length < 6) {
        showToast('Þifre en az 6 karakter olmalıdır!', 'warning');
        return;
    }
    if (submitBtn) {
        submitBtn.disabled = true;
    ...
    .then(data => {
        if (!data.success) {
            throw new Error(data.message || 'Kullanıcı oluþturulamadı.');
        }
        showToast(data.message, 'success');
    ...
    .catch(error => {
        showToast('Kullanıcı oluþturulurken hata oluþtu: ' + error.message, 'error');
    })
    .finally(() => {
        if (submitBtn) {
            submitBtn.disabled = false;
            submitBtn.innerHTML = '<i class="fas fa-save me-2"></i>Kullanıcı Oluþtur';
        }
    });
}
...
function uploadBulkUsers() {
    ...
    if (!fileAd.endsWith('.xlsx') && !fileAd.endsWith('.xls')) {
        showToast('Sadece Excel dosyaları (.xlsx, .xls) yüklenebilir!', 'error');
        return;
    }
    // Progress bar göster
        ...
        if (data.success) {
            ...
            let resultHTML = `
                <div class="alert alert-${data.error_count > 0 ? 'warning' : 'success'}" role="alert">
                    ...
                    <hr>
                    <p class="mb-2">
                        <strong>${data.success_count}</strong> kullanıcı baþarıyla eklendi.
...
function handleLogoYükle(input) {
    if (input.files && input.files[0]) {
        ...
        if (file.size > 16 * 1024 * 1024) {
            showToast('Dosya boyutu 16MB\'dan büyük olamaz!', 'error');
            input.value = '';
            return;
        }
        // Dosya türü kontrolü
        ...
        if (!allowedTips.includes(file.type)) {
            showToast('Geçersiz dosya formatı! Sadece PNG, JPG, JPEG, GIF, SVG, WEBP desteklenir.', 'error');
            input.value = '';
            return;
        }
        // Logo yükleme
...
function handleDüzenleProfilPhotoYükle(input) {
    if (input.files && input.files[0]) {
        ...
        .catch(error => {
            showToast('Yükle hatası: ' + error.message, 'error');
        })
        .finally(() => {
            input.disabled = false;
        });
...
function deleteKalitePolitikasi(id) {
...
function showInlineBanner(message) {
    try {
        const bannerId = 'inline-banner-fallback';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof bootstrap === 'undefined') {
        ...
        showInlineBanner('Bootstrap yüklenmemiş. Sayfayı yenileyin.');
        return;
    }
    ...
    fetch('/api/rol-matrisi')
        ...
        .then(data => {
            ...
            if (data.success === true && data.kullanicilar && data.yetki_kategorileri) {
                // success true ise ve gerekli veriler varsa render et
        ...
        <div class="table-responsive" style="max-height: 650px; overflow: auto;">
            ...
            yetkiKategorileri.forEach(kat => {
                ...
                Object.keys(crudGroups).forEach(baseAd => {
                    ...
                    const groupAd = group[0].ad.replace(/ (Oluþtur|Oku|Güncelle|Sil)$/, '');
            ...
            yetkiKategorileri.forEach(kat => {
                ...
                Object.keys(crudGroups).forEach(baseAd => {
                    ...
                    crudOrder.forEach(crud => {
                        ...
                        if (yetki) {
                            const crudAd = crud === 'c' ? 'Oluþtur' : crud === 'r' ? 'Oku' : crud === 'u' ? 'Güncelle' : 'Sil';
...
function saveRolMatrisi() {
    showToast('success', 'Rol matrisi kaydedildi!');
    loadRolMatrisi();
}
...
// Fallbacklar: ana script çökse bile temel kurum işlemleri çalışsın
if (typeof window.confirmDelete !== 'function') {
    window.confirmDelete = async function(title, text) {
        if (typeof Swal !== 'undefined') {
            const result = await Swal.fire({
                title: title || 'Emin misiniz?',
                text: text || '',
...
if (typeof window.viewOrganization !== 'function') {
    window.viewOrganization = async function(orgAd) {
        try {
            const res = await fetch(`/admin/get-organization/${orgAd}`);
            const data = await res.json();
            if (!data.success) throw new Error(data.message || 'Kurum getirilemedi');
            ...
            if (modalEl) {
                ...
                if (typeof window.showToast === 'function') window.showToast('info', `Kurum: ${data.organization.kisa_ad}`, 'Bilgi');
                else showInlineBanner(`Kurum: ${data.organization.kisa_ad}`);
            }
        } catch (err) {
            if (window.showToast) showToast('error', err.message || 'Kurum getirilemedi');
            else showInlineBanner(err.message || 'Kurum getirilemedi');
        }
    };
}

if (typeof window.editOrganization !== 'function') {
    window.editOrganization = async function(orgAd) {
        try {
            const res = await fetch(`/admin/get-organization/${orgAd}`);
            const data = await res.json();
            if (!data.success) throw new Error(data.message || 'Kurum getirilemedi');
            ...
            if (window.showToast) showToast('error', err.message || 'Kurum düzenleme açılmadı');
            else showInlineBanner(err.message || 'Kurum düzenleme açılmadı');
        }
    };
}

if (typeof window.deleteOrganization !== 'function') {
    window.deleteOrganization = async function(orgAd, orgId) {
        const confirmed = await window.confirmDelete?.(`"${orgAd}" kurumunu silmek istediğinize emin misiniz?`, 'Bu işlem geri alınamaz ve kurumdaki tüm kullanıcılar, süreçler ve veriler silinecektir!');
        if (!confirmed) return;
        const csrf = document.querySelector('meta[name="csrf-token"]')?.content || '';
        try {
            const res = await fetch(`/admin/delete-organization/${orgId}`, { method: 'DELETE', headers: { 'Content-Type': 'application/json', 'X-CSRFToken': csrf } });
            const data = await res.json();
            if (!data.success) throw new Error(data.message || 'Kurum silinemedi');
            if (window.showToast) showToast('success', data.message || 'Kurum silindi');
            setTimeout(() => location.reload(), 1200);
        } catch (err) {
            if (window.showToast) showToast('error', err.message || 'Kurum silinemedi');
            else showInlineBanner(err.message || 'Kurum silinemedi');
        }
    };
}
...
document.addEventListener('DOMContentLoaded', function() {
    const rolMatrisiTab = document.getElementById('rol-matrisi-tab');
    if (rolMatrisiTab) {
        // Bootstrap tab event'ini kullan
        rolMatrisiTab.addEventListener('shown.bs.tab', function(e) {
            loadRolMatrisi();
        });
        // Ýlk yüklemede eÝer aktif tab ise yükle
        const rolMatrisiPane = document.getElementById('rol-matrisi');
        if (rolMatrisiPane && rolMatrisiPane.classList.contains('active')) {
            loadRolMatrisi();
        }
    }
});
