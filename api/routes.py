...
def api_surec_karne_performans(surec_id):
    ...
    try:
        ...
        debug_mode = request.args.get('debug', '0') == '1'  # Debug modu
        current_app.logger.info(f"Yıl: {yil}, Periyot: {periyot}, Ay: {ay}, Debug: {debug_mode}")
        ...
        gostergeler = []
        hata_listesi = []  # Debug için hata listesi
        for pg in surec_pgler:
            try:
                ...
                import traceback
                error_msg = f"PG {pg.id} ({pg.ad}): {str(pg_error)}"
...
def api_surec_karne_faaliyetler(surec_id: int):
    ...
    try:
        ...
        # Kullanıcının bu süreçteki bireysel faaliyetlerini (varsa) topla
        surec_faaliyet_ids = [f.id for f in surec_faaliyetler]
        bireysel_map: dict[int, BireyselFaaliyet] = {}
        if surec_faaliyet_ids:
            bireyseller = BireyselFaaliyet.query.filter(
                BireyselFaaliyet.user_id == current_user.id,
        ...
        # Aylık takip verilerini toplu çek
        takip_map: dict[int, list[dict]] = {}
        bireysel_ids = [b.id for b in bireysel_map.values()]
        if bireysel_ids:
            takip_rows = FaaliyetTakip.query.filter(
                FaaliyetTakip.user_id == current_user.id,
        ...
        for faaliyet in surec_faaliyetler:
            ...
            faaliyetler.append({
                # Frontend beklentisi: id bireysel faaliyet id'si (yoksa 0)
                'id': bireysel_id,
...
@api_bp.route('/surec/<int:surec_id>/faaliyet/<int:surec_faaliyet_id>/create-bireysel', methods=['POST'])
@login_required
@csrf.exempt
def api_create_bireysel_faaliyet_from_surec(surec_id: int, surec_faaliyet_id: int):
    """Süreç faaliyetinden mevcut kullanıcıya bireysel faaliyet oluştur (idempotent)."""
    try:
        # Süreç + yetki kontrolü (görüntüleme ile aynı)
        ...
        if mevcut:
            return jsonify({'success': True, 'bireysel_faaliyet_id': mevcut.id})
...
@api_bp.route('/faaliyet/<int:faaliyet_id>/takip', methods=['POST'])
@login_required
@csrf.exempt
def api_faaliyet_takip_kaydet(faaliyet_id: int):
    """Bireysel faaliyetin aylık takibini kaydet."""
    try:
        ...
        if ay < 1 or ay > 12:
            return jsonify({'success': False, 'message': 'Ay 1-12 aralığında olmalı'}), 400
        gerceklesti = bool(data.get('gerceklesti'))

        faaliyet = BireyselFaaliyet.query.get(faaliyet_id)
        if not faaliyet:
            return jsonify({'success': False, 'message': 'Faaliyet bulunamadı'}), 404
        if faaliyet.user_id != current_user.id:
            return jsonify({'success': False, 'message': 'Bu faaliyet üzerinde yetkiniz yok'}), 403
...
def api_surec_karne_kaydet(surec_id):
    ...
    try:
        ...
        for veri in pg_verileri:
            ...
            # Hangi kullanıcı adına veri girileceği (gönderilmezse kendisi)
            veri_sahibi_id = veri.get('user_id') or veri.get('kullanici_id')
            veri_sahibi_id = int(veri_sahibi_id) if veri_sahibi_id else int(current_user.id)

            # Admin dışında kurum dışına yazma engeli
            if current_user.sistem_rol != 'admin':
                veri_sahibi = User.query.get(veri_sahibi_id)
                if not veri_sahibi:
                    return jsonify({'success': False, 'message': 'Kullanıcı bulunamadı'}), 404
                if veri_sahibi.kurum_id != current_user.kurum_id:
                    return jsonify({'success': False, 'message': 'Bu kullanıcı üzerinde işlem yetkiniz yok'}), 403

            # Süreç PG doğrulaması
            surec_pg = None
            if surec_pg_id > 0:
                surec_pg = SurecPerformansGostergesi.query.get(surec_pg_id)
                if not surec_pg or surec_pg.surec_id != surec_id:
                    return jsonify({'success': False, 'message': 'Geçersiz süreç performans göstergesi'}), 400
            ...
            if pg_id == 0 and surec_pg_id > 0:
                ...
                # Bireysel kaydı oluştur (ilgili kullanıcı adına)
            ...
            # Bireysel PG doğrulaması (süreç dışına yazmayı engelle)
            ...
            if bireysel_pg.user_id != veri_sahibi_id:
                return jsonify({'success': False, 'message': 'Bu gösterge üzerinde yetkiniz yok'}), 403
            ...
            if bireysel_pg.kaynak == 'Süreç':
                ...
                return jsonify({'success': False, 'message': 'Bu gösterge süreç karnesine ait değil'}), 400
