...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="dropdown">
            <button class="btn btn-primary dropdown-toggle" type="button" id="addWidgetBtn" data-bs-toggle="dropdown" aria-expanded="false">
                <i class="fas fa-plus me-1"></i> Widget Ekle / Aç
            </button>
            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="addWidgetBtn" id="hiddenWidgetsMenu">
                <li><h6 class="dropdown-header">Gizli Widgetlar</h6></li>
                <li id="noHiddenWidgetsMsg"><span class="dropdown-item text-muted">Tüm widgetlar açık</span></li>
            </ul>
        </div>
    </div>
            ...
            <div class="col-md-4 widget-wrapper" data-id="{{ widget.id }}">
                <div class="card card-v3 h-100">
                    <div class="card-header card-header-v3 d-flex justify-content-between align-items-center handle">
                        <h5 class="mb-0">
                            {% if widget.id == 'widget-1' %}<i class="fas fa-bolt me-2"></i>Hızlı İşlemler
                            ...
                            {% elif widget.id == 'widget-5' %}<i class="fas fa-hourglass-half me-2"></i>Yaklaşan Teslimatlar
                            {% elif widget.id == 'widget-6' %}<i class="fas fa-bell me-2"></i>Bildirimler
                            {% elif widget.id == 'widget-7' %}<i class="fas fa-bolt me-2 text-warning"></i>Hızlı Veri
                            {% elif widget.id == 'widget-8' %}<i class="fas fa-project-diagram me-2"></i>Proje Vitrini
                            {% endif %}
                        </h5>
                        <button class="btn btn-sm btn-light rounded-circle close-widget-btn" onclick="hideWidget('{{ widget.id }}')" title="Widget'ı Gizle">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                    <div class="card-body">
                        {% if widget.id == 'widget-1' %}
                            <!-- Widget 1: Hızlı İşlemler -->
                            <div class="d-grid gap-2">
                                <button class="btn btn-outline-primary text-start p-3 shadow-sm border-0 bg-light-hover">
                                    ...
                                    <span class="fw-bold text-dark">Yeni Görev Oluştur</span>
                                </button>
                                <button class="btn btn-outline-success text-start p-3 shadow-sm border-0 bg-light-hover">
                                    ...
                                    <span class="fw-bold text-dark">Toplantı Planla</span>
                                </button>
                        ...
                        {% elif widget.id == 'widget-2' %}
                            ...
                            <div class="list-group list-group-flush">
                                ...
                                <a href="#" class="list-group-item list-group-item-action d-flex justify-content-between align-items-center px-0 border-0 mb-2 rounded bg-white shadow-sm p-3">
                                    <div class="d-flex align-items-center">
                                        <i class="far fa-check-circle text-success me-3 fa-lg"></i>
                                        <span class="fw-medium">{{ task.name }}</span>
                                    </div>
                                    <span class="badge bg-soft-primary text-primary rounded-pill px-3">{{ task.priority }}</span>
                                </a>
                                {% else %}
                                <div class="text-center text-muted py-4">
                                    <i class="fas fa-check-circle fa-3x mb-3 text-light-gray"></i>
                                    <p class="mb-0">Bekleyen önemli bir görev yok.</p>
                                </div>
                                {% endfor %}
                            </div>
                            <div class="mt-3 text-center">
                                <a href="#" class="btn btn-sm btn-outline-secondary rounded-pill px-4">Tümünü Gör</a>
                            </div>

                        {% elif widget.id == 'widget-3' %}
                            <!-- Widget 3: Karalama Defteri -->
                            <div class="form-group h-100 d-flex flex-column">
                                <textarea class="form-control bg-light border-0 flex-grow-1 p-3 shadow-inner" id="postit-content" style="resize: none; border-radius: 10px;" placeholder="Notlarınızı buraya alın...">{{ postit_content }}</textarea>
                        ...
                        {% elif widget.id == 'widget-4' %}
                            <!-- Widget 4: Stratejik Özet (Chart.js) -->
                            <div class="position-relative text-center" style="height: 200px; display: flex; align-items: center; justify-content: center;">
                                ...
                                <div class="position-absolute" style="bottom: 20px; left: 0; right: 0; text-align: center;">
                                    ...
                                    <div class="small text-muted">Genel Başarım</div>
                                </div>
                            </div>
                            <div class="row mt-3 pt-3 border-top">
                                <div class="col-6 border-end text-center">
                                    <div class="text-success h4 fw-bold mb-0">{{ stats.completed_tasks }}</div>
                                    <small class="text-muted fw-bold text-uppercase" style="font-size: 0.7rem;">Tamamlandı</small>
                                </div>
                                <div class="col-6 text-center">
                                    <div class="text-warning h4 fw-bold mb-0">{{ stats.pending_tasks }}</div>
                                    <small class="text-muted fw-bold text-uppercase" style="font-size: 0.7rem;">Beklemede</small>
                                </div>
                            </div>

                        {% elif widget.id == 'widget-5' %}
                            <!-- Widget 5: Yaklaşan Teslimatlar -->
                            <div class="d-flex align-items-center justify-content-center flex-column h-100 py-3">
                                <div class="countdown-timer mb-3 p-4 rounded-circle bg-soft-danger d-flex flex-column align-items-center justify-content-center" style="width: 120px; height: 120px;">
                                    <span class="display-4 fw-bold text-danger lh-1">02</span>
                                    <span class="small fw-bold text-danger text-uppercase">Gün</span>
                                </div>
                        ...
                        {% elif widget.id == 'widget-6' %}
                            <!-- Widget 6: Bildirimler -->
                            <div class="notification-list" style="max-height: 250px; overflow-y: auto;">
                                <ul class="list-unstyled mb-0">
                                    {% for notif in notifications %}
                                    <li class="d-flex align-items-start mb-3 pb-3 border-bottom last-no-border">
                                        <div class="flex-shrink-0 me-3">
                                            <div class="icon-circle bg-light-{{ notif.type }} text-{{ notif.type }} rounded-circle d-flex align-items-center justify-content-center" style="width: 40px; height: 40px;">
                                                <i class="fas {{ notif.icon }}"></i>
                                            </div>
                                        </div>
                                        <div class="flex-grow-1">
                                            <p class="mb-1 fw-medium text-dark">{{ notif.message }}</p>
                        ...
                        {% elif widget.id == 'widget-7' %}
                            <!-- Widget 7: Hızlı Veri Girişi -->
                            <div class="table-responsive">
                                <table class="table table-borderless align-middle mb-0">
                                    <thead class="bg-light">
                                        <tr>
                                            <th class="ps-3 text-muted small text-uppercase">Gösterge</th>
                                            <th class="text-muted small text-uppercase">Hedef</th>
                                            <th class="text-end pe-3 text-muted small text-uppercase">Giriş</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        <tr>
                                            <td class="ps-3">
                                                <div class="fw-bold text-dark">{{ pg.name }}</div>
                                                <small class="text-muted">{{ pg.unit }}</small>
                                            </td>
                                            <td><span class="badge bg-light text-dark border">{{ pg.target }}</span></td>
                                            <td class="text-end pe-3">
                                                <div class="input-group input-group-sm justify-content-end">
                                                    <input type="number" class="form-control" placeholder="Değer" style="max-width: 80px;">
                                                    <button class="btn btn-primary" type="button" onclick="alert('Veri Kaydedildi!')">
                        ...
                        {% elif widget.id == 'widget-8' %}
                            <!-- Widget 8: Proje Vitrini -->
                            <div class="project-list">
                                {% for project in active_projects %}
                                <div class="mb-3">
                                    <div class="d-flex justify-content-between mb-1">
                                        <span class="fw-bold text-dark small">{{ project.name }}</span>
                                        <span class="text-{{ project.color }} small fw-bold">{{ project.progress }}%</span>
                                    </div>
                                    <div class="progress" style="height: 6px; border-radius: 3px;">
                                        <div class="progress-bar bg-{{ project.color }}" role="progressbar" style="width: {{ project.progress }}%"></div>
                                    </div>
                                </div>
                                {% endfor %}
                            </div>
...
<script>
    ...
    function updateHiddenMenu() {
        ...
        if (hiddenWidgets.length === 0) {
            menu.innerHTML += '<li id="noHiddenWidgetsMsg"><span class="dropdown-item text-muted">Tüm widgetlar açık</span></li>';
        } else {
            hiddenWidgets.forEach(w => {
...
<style>
    ...
    /* New Header Styles */
    .card-header-v3 {
        background-color: #2c3e50; /* Premium Dark Blue */
        color: white;
        font-weight: 600;
...
with open(r'c:\SPY_Cursor\SP_Code\templates\v3\dashboard.html', 'w', encoding='utf-8') as f:
    f.write(content)
