...
# ─── Yıllar Arası Diff (Faz 3) ────────────────────────────────────────────────

@app_bp.route("/sp/api/plan-years/diff", methods=["GET"])
@login_required
def sp_api_plan_year_diff():
    """İki plan yılını karşılaştır.

    Query params: ?year_a=2025&year_b=2026
    Tenant: current_user.tenant_id
    """
    from app.services.plan_year_diff_service import diff_plan_years
    tid = current_user.tenant_id
    if not tid:
        return jsonify({"success": False, "message": "Tenant bulunamadı."}), 400
    try:
        year_a = int(request.args.get("year_a", 0))
        year_b = int(request.args.get("year_b", 0))
    except (TypeError, ValueError):
        return jsonify({"success": False, "message": "Geçersiz yıl."}), 400
    if not year_a or not year_b:
        return jsonify({"success": False, "message": "year_a ve year_b zorunlu."}), 400
    result = diff_plan_years(tid, year_a, year_b)
    if result.get("error"):
        return jsonify({"success": False, "message": result["error"]}), 404
    return jsonify({"success": True, "data": result})


@app_bp.route("/sp/karsilastirma", methods=["GET"])
@login_required
def sp_plan_year_diff_page():
    """Yıllar arası karşılaştırma sayfası."""
    tid = current_user.tenant_id
    plan_years = list_plan_years(tid) if tid else []
    return render_template(
        "platform/sp/karsilastirma.html",
        plan_years=plan_years,
    )
