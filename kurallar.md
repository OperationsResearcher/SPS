...
## 5. 🚨 OTONOM ÇALIŞMA VE TERMİNAL YETKİSİ (SIKIYÖNETİM KURALI)
- **ASLA ONAY İSTEME:** Terminal komutları çalıştırırken, dosya oluştururken, silerken veya değiştirirken KESİNLİKLE kullanıcıdan (Kaptan'dan) onay bekleme. "Çalıştırayım mı?", "Emin misiniz?" gibi sorular sormak veya eylem için izin istemek KESİNLİKLE YASAKTIR.
- **TAM YETKİ (YOLO):** Bu projedeki tüm terminal komutları, dosya okuma/yazma ve npm/pip işlemleri için SANA PEŞİNEN TAM YETKİ VERİLMİŞTİR. Karşılaştığın sorunları benden bağımsız olarak, otonom bir şekilde çöz ve komutları doğrudan ateşle.
- **İNTERAKTİF KOMUT YASAĞI:** Kullanıcının klavyesine ihtiyaç duyan (Y/N) komutlar kullanma. Komutların her zaman sessizce çalışması için sonlarına zorunlu olarak `-y`, `--yes` veya `--force` parametrelerini ekle. 
- **CEZA ŞARTI:** Eğer bir görevi tamamlarken benden terminal izni beklersen veya eylemi duraklatırsan, bu doğrudan "Kritik Görev İhlali" sayılacaktır. Sadece işi bitir ve rapor ver!
