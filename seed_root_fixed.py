...
def seed_database():
    with app.app_context():
        ...
        # 3. YETKİ MATRİSİ ÖRNEK (Role tablosu olmadığı için matris tablosuna ek gerekebilir ama şimdilik user.sistem_rol yeterli)
        ...
        ana_strat = AnaStrateji(
            ...
            code="S1", # kod -> code
            kurum_id=kurum_kmf.id
            # yil=2025 # parametresi modelde yok, silindi
        )
        ...
        db.session.commit() # Rolleri kaydet
        ...
        sr4 = Surec(
            ...
            code="SR4", # kod -> code
            kurum_id=kurum_kmf.id 
            # alt_strateji_id? Modelde relation var ama column adı ne?
            # models/process.py'de alt_stratejiler ManyToMany relation.
            # Ancak process.py'de ForeignKey definition yok gibi, secondary table var.
            # Ama models/process.py'de alt_stratejiler ilişkisi var.
            # Fakat models/process.py'de SurecPerformansGostergesi -> alt_strateji_id var.
            # Surec -> alt_strateji ilişkisi process.py:81'de secondary table olarak var.
        )
        
        # Many-To-Many ilişkiler
        sr4.liderler.append(lider1)
        sr4.uyeler.append(lider1)
        sr4.alt_stratejiler.append(alt_strat) # M2M
