(function(){
  ...
  async function load(){
    try {
      const j = await (await fetch('/raporlar/api/yatirimci-sunum/preview',{credentials:'same-origin'})).json();
  ...
  document.getElementById('dl').addEventListener('click', async()=>{
    ...
    try {
      const r=await fetch('/raporlar/api/yatirimci-sunum/generate',{credentials:'same-origin'});
