(function(){
  ...
  async function load(){
    try {
      const j = await (await fetch('/k-report/api/ai-presentation/preview',{credentials:'same-origin'})).json();
  ...
  document.getElementById('dl-btn').addEventListener('click', async () => {
    ...
    try {
      const r = await fetch('/k-report/api/ai-presentation/generate', {method:'GET', credentials:'same-origin'});
      if (!r.ok) {
