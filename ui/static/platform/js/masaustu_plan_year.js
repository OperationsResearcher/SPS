/**
 * masaustu_plan_year.js — Masaüstü SP Dönem Yönetimi
 * Bağımlılık: SweetAlert2
 */
(function () {
  "use strict";

  const root = document.getElementById("masaustu-plan-year-root");
  if (!root) return;

  const PLAN_YEARS_URL    = root.dataset.planYearsUrl    || "";
  ...
  const yearSelect  = document.getElementById("mpy-year-select");
  const btnNew      = document.getElementById("mpy-btn-new");
  const btnClose    = document.getElementById("mpy-btn-close");
  const modalNew    = document.getElementById("mpy-modal-new");
  const btnSave     = document.getElementById("mpy-btn-save");
  ...
  if (btnNew && modalNew && CAN_MANAGE) {
    btnNew.addEventListener("click", openModal);
  }

  document.querySelectorAll("[data-modal-close='mpy-modal-new']").forEach(function (el) {
  ...
  if (btnSave && CAN_MANAGE) {
    btnSave.addEventListener("click", async function () {
      const yearInput     = document.getElementById("mpy-year");
      const nameInput     = document.getElementById("mpy-name");
      const fromYearInput = document.getElementById("mpy-from-year");
