...
(function () {
  ...
  const K_VEKTOR_ENABLED = indexRoot.dataset.kVektorEnabled === "true";
      ...
      subs.forEach((ss) => {
        ...
        if (K_VEKTOR_ENABLED) {
          ...
          row.innerHTML =
            ...
            `<input type="checkbox" class="ss-check" id="${id}" value="${ss.id}">` +
            `<span>${(ss.code || "·") + " — " + (ss.title || "")}</span></label>` +
            `<input type="number" class="mc-input ss-pct-input" data-ss-id="${ss.id}" min="0" max="100" step="0.01" placeholder="%" style="width:96px;flex-shrink:0;" />`;
  ...
  async function openEditModal(processId, triggerEl) {
    ...
    try {
      ...
      const linkPct = {};
      (p.sub_strategy_links || []).forEach((l) => {
        linkPct[Number(l.sub_strategy_id)] = l.contribution_pct;
      });
      document.querySelectorAll("#modal-surec-add .ss-check").forEach((cb) => {
        cb.checked = linkIds.includes(parseInt(cb.value, 10));
      });
      if (K_VEKTOR_ENABLED) {
        document.querySelectorAll("#modal-surec-add .ss-pct-input").forEach((inp) => {
          const sid = parseInt(inp.getAttribute("data-ss-id"), 10);
          const v = linkPct[sid];
          inp.value = v != null && v !== "" && !Number.isNaN(Number(v)) ? String(v) : "";
        });
      }
  ...
  async function saveProcessForm() {
    ...
    if (K_VEKTOR_ENABLED) {
      const links = [];
      for (const cb of document.querySelectorAll("#modal-surec-add .ss-check:checked")) {
      ...
      subStrategyLinksPayload = checked.map((sid) => ({ sub_strategy_id: sid }));
    }
    ...
    const body = {
      ...
      sub_strategy_links: subStrategyLinksPayload,
    };
