/**
 * Kurum / Masa├╝st├╝ takvimi: bo┼ş g├╝ne t─▒klama, aral─▒k se├ğimi, h─▒zl─▒ olu┼şturma modallar─▒.
...
(function (global) {
  ...
  function boot(opts) {
    ...
    async function loadMeta() {
      ...
      if (!res.ok || !data.success) throw new Error(data.message || t("Meta yüklenemedi"));
      metaCache = data;
      return data;
    }
    ...
    function dateRangeRecap() {
      if (rangeStart === rangeEndIncl) return t("Takvim seçimi:") + " " + fmtTr(new Date(rangeStart + "T12:00:00"));
      return t("Takvim seçimi:") + " " + fmtTr(new Date(rangeStart + "T12:00:00")) + " ÔåÆ " + fmtTr(new Date(rangeEndIncl + "T12:00:00"));
    }
    ...
    function openModal(isoStart, isoEndExclusive) {
      ...
      modalTitle.textContent = t("Takvime ne ekleyelim?");
    ...
    function buildAssigneeCheckboxes(users, selectedIds, canMulti) {
      ...
      if (!users || !users.length) {
        box.innerHTML = `<div class="text-xs text-slate-500">${t("Süreçte atanabilir kullanıcı bulunamadı.")}</div>`;
        hint.textContent = "";
        return;
      }
      const sel = new Set((selectedIds || []).map((x) => String(x)));
      hint.textContent = canMulti
        ? t("Süreç lideri birden fazla kişi seçebilir.")
        : t("Yalnızca kendinize atama yapılabilir; sunucu kurallarıyla doğrulanır.");
      users.forEach((u) => {
    ...
    async function loadActivityMetaForProcess(pid) {
      ...
      try {
        ...
        const data = await res.json();
        if (!res.ok || !data.success) throw new Error(data.message || t("Yüklenemedi"));
    ...
    function applyActivityMeta(data) {
      ...
      sel.innerHTML = `<option value="">${t("Bağımsız")}</option>`;
      kpis.forEach((k) => {
        const o = document.createElement("option");
        o.value = String(k.id);
        o.textContent = k.label;
    ...
    selectProcess.addEventListener("change", () => {
      ...
      if (!v) {
        actMetaBox.classList.add("hidden");
        actMetaHint.textContent = t("Önce süreç seçin.");
        actMetaHint.classList.remove("hidden");
        return;
      }
    ...
    function showForm(mode) {
      ...
      if (mode === "process") {
        ...
        modalTitle.textContent = t("Yeni Faaliyet");
        ...
        modalTitle.textContent = t("Yeni görev");
        modalBadge.classList.remove("hidden");
        modalBadge.innerHTML = `<span class="text-blue-600 dark:text-blue-400">${t("Proje görevi formu ile aynı")}</span>`;
        resetProjectForm();
        fillProjectFormFromMeta(metaCache);
      } else {
        modal.classList.add("cal-modal--self");
        headIcon.className = "fas fa-user text-amber-500";
        modalTitle.textContent = t("Kendime görev");
        modalBadge.classList.remove("hidden");
        modalBadge.innerHTML = `<span class="text-amber-600 dark:text-amber-400">${t("Bireysel faaliyet")}</span>`;
        resetSelfForm();
    ...
    async function submitForm() {
      ...
      try {
        if (formMode === "self") {
          ...
          if (!title) throw new Error(t("Başlık girin."));
          ...
          if (ed < sd) throw new Error(t("Bitiş tarihi başlangıçtan önce olamaz."));
          ...
          const data = await res.json();
          if (!res.ok || !data.success) throw new Error(data.message || t("Kaydedilemedi"));
        } else if (formMode === "process") {
          const pid = parseInt(selectProcess.value, 10);
          if (!pid) throw new Error(t("Süreç seçin."));
          if (actMetaBox.classList.contains("hidden")) throw new Error(t("Süreç için form yüklenemedi."));
          const name = (document.getElementById("cal-act-name").value || "").trim();
          if (!name) throw new Error(t("Faaliyet adı zorunludur."));
          const startAt = document.getElementById("cal-act-start").value;
          const endAt = document.getElementById("cal-act-end").value;
          if (!startAt || !endAt) throw new Error(t("Başlangıç ve bitiş tarih/saat zorunludur."));
          if (endAt < startAt) throw new Error(t("Bitiş, başlangıçtan sonra olmalıdır."));
          const assigneeIds = Array.from(document.querySelectorAll(".cal-act-assignee-chk:checked"))
            .map((o) => parseInt(o.value, 10))
            .filter((x) => !Number.isNaN(x));
          if (!assigneeIds.length) throw new Error(t("En az bir atanan seçin."));
          ...
          if (!pid) throw new Error(t("Proje seçin."));
          const title = (document.getElementById("cal-proj-title").value || "").trim();
          if (!title) throw new Error(t("Başlık zorunludur."));
          const body = {
            project_id: pid,
            title,
